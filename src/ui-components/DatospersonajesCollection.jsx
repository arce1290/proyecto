...
import { listPersonajes } from "../graphql/queries";
import Datospersonajes from "./Datospersonajes";
...
export default function DatospersonajesCollection(props) {
    ...
    while ((newCache.length < cacheUntil || !isPaginated) && newNext != null) {
      ...
      const result = (
        await client.graphql({
          query: listPersonajes.replaceAll("__typename", ""),
          variables,
        })
      ).data.listPersonajes;
  ...
  return (
    <div>
      <Collection
        ...
        {...getOverrideProps(overrides, "DatospersonajesCollection")}
        {...rest}
      >
        {(item, index) => {
          if (loading) {
